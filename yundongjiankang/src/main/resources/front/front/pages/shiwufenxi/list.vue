<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.shiwufenxiName" type="text" placeholder="标题"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in shiwufenxiTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.shiwufenxiName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.shiwufenxiPhoto?product.shiwufenxiPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.shiwufenxiNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.shiwufenxiNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('shiwufenxi','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('shiwufenxi','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('shiwufenxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 食物类型 字段 字典表查询方法
		  	let shiwufenxiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'shiwufenxi_types'
		  	});
		  	this.shiwufenxiTypesList = shiwufenxiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.shiwufenxiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.shiwufenxiTypes = this.categoryName
                            }

                let res = await this.$api.list(`shiwufenxi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shiwufenxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.shiwufenxiName) {
                    searchForm['shiwufenxiName'] = this.searchForm.shiwufenxiName
                }
				if (this.searchForm.shiwufenxiName) {
					searchForm['shiwufenxiName'] = this.searchForm.shiwufenxiName
				}

                let res = await this.$api.list(`shiwufenxi`, searchForm);
				// 如果是第一页数据置空
