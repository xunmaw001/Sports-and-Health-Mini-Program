<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yundongmokuaiName" type="text" placeholder="运动名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in yundongmokuaiTypesList" :key="index"
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.yundongmokuaiName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.yundongmokuaiPhoto?product.yundongmokuaiPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.yundongmokuaiNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
            <text class="">￥{{product.yundongmokuaiNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('yundongmokuai','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('yundongmokuai','删除')" class="cuIcon-delete"
                  @click.stop="onDeleteTap(product.id)">删除</text>
        </view>
    </view>
</view>	<button
            :style='{"boxShadow":"0 0 0px rgba(0,0,0,0) inset","backgroundColor":"#B24592","borderColor":"rgba(57, 78, 99, 1)","borderRadius":"40rpx","color":"rgba(0, 0, 0, 1)","borderWidth":"0","width":"40%","fontSize":"28rpx","borderStyle":"solid","height":"76rpx"}'
            v-if="isAuth('yundongmokuai','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 运动运动类型 字段 字典表查询方法
		  	let yundongmokuaiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'yundongmokuai_types'
		  	});
		  	this.yundongmokuaiTypesList = yundongmokuaiTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.yundongmokuaiName = "";
			},
			//类别搜索
				...
				let params = {
					...
					limit: mescroll.size
							...
							if (this.categoryName != 0) {
                                params.yundongmokuaiTypes = this.categoryName
                            }

                let res = await this.$api.list(`yundongmokuai`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yundongmokuai', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.yundongmokuaiName) {
                    searchForm['yundongmokuaiName'] = this.searchForm.yundongmokuaiName
                }
				if (this.searchForm.yundongmokuaiName) {
					searchForm['yundongmokuaiName'] = this.searchForm.yundongmokuaiName
				}

                let res = await this.$api.list(`yundongmokuai`, searchForm);
				// 如果是第一页数据置空
