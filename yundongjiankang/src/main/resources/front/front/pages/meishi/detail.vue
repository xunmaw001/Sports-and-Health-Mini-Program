		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.meishiNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.meishiOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.meishiOldMoney}}</text></text>
                    	</text>
					</view>
				</view>
					<view class="name shop-title"
						  :style='{"boxShadow":"0 0 6rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"#fff","backgroundColor":"#fff","color":"#E57498","borderRadius":"0","borderWidth":"0","fontSize":"36rpx","borderStyle":"solid"}'>
						商品名称：{{detail.meishiName}}
					</view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							  :style='{"width":"auto","fontSize":"28rpx","color":"#E57498","textAlign":"left"}'>
							商品类型：</view>
						<view class="text" :style='{"color":"#E57498","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.meishiValue}}</view>
					</view>
					<view class="detail-list-item"
						  :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'>
						<view class="lable"
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.meishiContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('meishi', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.meishiPhoto ? this.detail.meishiPhoto.split(",") : [];
			},
			//添加购物车
            async onCartTap() {
            ...
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        meishiId: this.detail.id,
                        meishiName: this.detail.meishiName,
                        meishiPhoto: this.swiperList[0],
                    buyNumber: 1,
                        meishiNewMoney: this.detail.meishiNewMoney,
                }])
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('meishiCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					meishiId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../meishiCommentback/add-or-update?meishiId=${this.id}`)
            },
		}
	}
</script>
