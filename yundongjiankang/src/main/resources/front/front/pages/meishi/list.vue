	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.meishiName" type="text" placeholder="商品名称"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in meishiTypesList" :key="index"
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				...
				<view class="image-view">
					<image :style='{"borderRadius":"0px","width":"100%","height":"100%"}' mode="aspectFill"
						class="uni-product-image" :src="product.meishiPhoto?product.meishiPhoto.split(',')[0]:''">
					</image>
				</view>
				...
				<view class="uni-product-price" v-if="product.meishiNewMoney != null"
					:style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
					<text class="">￥{{product.meishiNewMoney}}</text>
				</view>
				<view style="display: flex;justify-content: space-between;">
					<text v-if="isAuth('meishi','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('meishi','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('meishi','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			let meishiTypes = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode: 'meishi_types'
			});
			this.meishiTypesList = meishiTypes.data.list;
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.meishiTypes = this.categoryName
				}

				let res = await this.$api.list(`meishi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('meishi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.meishiName) {
					searchForm['meishiName'] = this.searchForm.meishiName
				}
				if (this.searchForm.meishiName) {
					searchForm['meishiName'] = this.searchForm.meishiName
				}

				let res = await this.$api.list(`meishi`, searchForm);
				// 如果是第一页数据置空
