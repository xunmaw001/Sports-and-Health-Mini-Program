...
public class MeishiCommentbackController {
    ...
    */
    ...
    public R update(@RequestBody MeishiCommentbackEntity meishiCommentback, HttpServletRequest request){
...
//            meishiCommentback.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<MeishiCommentbackEntity> queryWrapper = new EntityWrapper<MeishiCommentbackEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        MeishiCommentbackEntity meishiCommentbackEntity = meishiCommentbackService.selectOne(queryWrapper);
        meishiCommentback.setUpdateTime(new Date());
        if(meishiCommentbackEntity==null){
...
//                            meishiCommentbackEntity.setMeishiId(Integer.valueOf(data.get(0)));   //商品 要改的
...
//                            meishiCommentbackEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
